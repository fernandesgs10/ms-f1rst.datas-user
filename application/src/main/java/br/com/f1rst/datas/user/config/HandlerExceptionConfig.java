...
public class HandlerExceptionConfig {
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException exception, WebRequest request) {
        ...
        errorDto.setDetailedMessages(exception.getBindingResult().getAllErrors().stream()
    ...
    public ResponseEntity<Object> handleAccessDeniedException(AccessDeniedException exception, WebRequest request) {
        ErrorDto errorDto = createErrorDto(HttpStatus.FORBIDDEN, exception, request);
        log.error("Access denied: {}", exception.getMessage(), exception);
        return ResponseEntity.status(HttpStatus.FORBIDDEN).body(errorDto);
    }

    @ExceptionHandler(GatewayException.class)
    public ResponseEntity<Object> handleIllegalStateException(GatewayException exception, WebRequest request) {
        ErrorDto errorDto = createErrorDto(HttpStatus.BAD_REQUEST, exception, request);
